...
dataset = MNISTRestore(subkey, '../datasets/mnist.npz', task=task)
dataset_test = MNISTRestore(subkey, '../datasets/mnist.npz', task=task, test=True)
...
gibbs_init = jax.jit(partial(gibbs_init, x0_shape=(28, 14, 1), ts=ts, fwd_sampler=fwd_sampler, dataset=dataset,
...
for k in range(args.ny0s):
    ...
    plt.imsave(f'./tmp_figs/mnist_{task}_{k}_pair_true.png', test_xy0[:, :, 0], cmap='gray')
    ...
    plt.imsave(f'./tmp_figs/mnist_{task}_{k}_init.png', normalise(dataset.concat(x0, test_y0))[:, :, 0], cmap='gray')
