...
#     u = jnp.linalg.cholesky(jnp.array([[0.5, 0.],
#                                        [0., 0.5]])) @ jax.random.normal(key_, (2,))
#     return jnp.array([(u[1] + y) * u[0],
#                       (u[0] - y) * u[1]])
def sampler_x_cond_y(key_, y):
    uu = 0.5 * jax.random.normal(key_, (2,))
    return uu + y


def sampler_y(key_):
    r = 0.5 * jax.random.normal(key_)
    return jax.random.choice(jax.random.split(key_)[1], jnp.array([-2., 2.])) + r
...
plt.scatter(xys[:, 0], xys[:, 1], s=1, alpha=0.5, label='p(x)')
plt.legend()
plt.show()

xs_cond_y0 = jax.vmap(sampler_x_cond_y, in_axes=[0, None])(keys, y0)
plt.scatter(xs_cond_y0[:, 0], xs_cond_y0[:, 1], s=1, alpha=0.5, label=f'p(x | y = {y0})')
plt.legend()
plt.show()
...
axes[0].scatter(xys[:, 0], xys[:, 1], s=1, alpha=0.5, label='True p(x)')
axes[1].scatter(approx_init_samples[:, 0], approx_init_samples[:, 1], s=1, alpha=0.5, label='Approx. p(x)')
axes[0].legend()
axes[1].legend()
...
key, subkey = jax.random.split(key)
fig, axes = plt.subplots(ncols=2, sharey='row', sharex='row')
axes[0].scatter(xs_cond_y0[:, 0], xs_cond_y0[:, 1], s=1, alpha=0.5, label=f'True p(x | y = {y0})')
axes[1].scatter(uss[:, -1, 0], uss[:, -1, 1], s=1, alpha=0.5, label=f'Approx. p(x | y = {y0})')
axes[0].legend()
axes[1].legend()
plt.tight_layout(pad=0.1)
plt.show()
