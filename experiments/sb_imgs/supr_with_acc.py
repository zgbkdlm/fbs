...
The aim is to validate the effect of accepting the path of Y
"""
import math
import argparse
import jax
...
def drift(xy, t):
    return nn_drift(xy, t, param_fwd)


def dispersion(t):
    return sde.dispersion(t)
...
def log_bwd(us, vs, mask_):
    uvs = dataset.concat(us, vs, mask_)
...
def log_fwd(xs, ys, mask_):
    xys = dataset.concat(xs, ys, mask_)
