r"""
Super-resolution experiment using the standard conditional score matching.
...
for k in range(args.ny0s):
    ...
    path_head_img = f'./imgs/results_supr/imgs/{dataset_name}-{sr_rate}{"-rm" if args.rnd_mask else ""}-{args.sde}-{k}'
    path_head_arr = f'./imgs/results_supr/arrs/{dataset_name}-{sr_rate}{"-rm" if args.rnd_mask else ""}-{args.sde}-{k}'
