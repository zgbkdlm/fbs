...
from fbs.dsb import ipf_fwd_loss, ipf_bwd_loss
...
mlp = MLP()
key, subkey = jax.random.split(key)
init_param_fwd, _, nn_eval_fwd = make_nn_with_time(mlp, dim_in=2, batch_size=10, key=subkey)
key, subkey = jax.random.split(key)
init_param_bwd, _, nn_eval_bwd = make_nn_with_time(mlp, dim_in=2, batch_size=10, key=subkey)
...
def opt_step_kernel_init(_b_param, _opt_state, _key):
    ...
    _key, _subkey = jax.random.split(_key)
    _loss, grad = jax.value_and_grad(ipf_fwd_loss)(_b_param, _, init_f, nn_eval_bwd, _x0s, ts, _subkey)
